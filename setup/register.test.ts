...
describe('parameterized SQL registration', () => {
  ...
  it('registers a group with parameterized query', () => {
    ...
    expect(row.trigger_pattern).toBe('@Romi');
  ...
  it('prevents SQL injection in JID field', () => {
    ...
    db.prepare(
      `INSERT OR REPLACE INTO registered_groups
       (jid, name, folder, trigger_pattern, added_at, container_config, requires_trigger)
       VALUES (?, ?, ?, ?, ?, NULL, ?)`,
    ).run(maliciousJid, 'Evil', 'evil', '@Romi', '2024-01-01T00:00:00.000Z', 1);
...
describe('file templating', () => {
  it('replaces assistant name in CLAUDE.md content', () => {
    let content = '# Romi\n\nYou are Romi, a personal assistant.';

    content = content.replace(/^# Romi$/m, '# Nova');
    content = content.replace(/You are Romi/g, 'You are Nova');
  ...
  it('handles names with special regex characters', () => {
    ...
    content = content.replace(/^# Romi$/m, `# ${newName}`);
    content = content.replace(/You are Romi/g, `You are ${newName}`);
...
describe('CLAUDE.md template copy', () => {
  ...
  function simulateRegister(
    ...
    if (assistantName !== 'Romi') {
      ...
      for (const mdFile of mdFiles) {
        ...
        content = content.replace(/^# Romi$/m, `# ${assistantName}`);
  ...
  it('copies global template for non-main group', () => {
    ...
    expect(content).toContain('You are Romi');
  ...
  it('custom name propagates to all channels and groups', () => {
    ...
    for (const folder of [
      ...
      expect(content).not.toContain('Romi');
    }
  });
