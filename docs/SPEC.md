...
export const ASSISTANT_NAME = process.env.ASSISTANT_NAME || 'Romi';
...
Messages must start with the trigger pattern (default: `@Romi`):
- `@Romi what's the weather?` → ✅ Triggers Claude
- `@andy help me` → ✅ Triggers (case insensitive)
- `Hey @Romi` → ❌ Ignored (trigger not at start)
...
[Jan 31 2:35 PM] John: @Romi what toppings do you recommend?
```
...
|---------|---------|--------|
| `@Assistant [message]` | `@Romi what's the weather?` | Talk to Claude |

### Commands Available in Main Channel Only

| Command | Example | Effect |
|---------|---------|--------|
| `@Assistant add group "Name"` | `@Romi add group "Family Chat"` | Register a new group |
| `@Assistant remove group "Name"` | `@Romi remove group "Work Team"` | Unregister a group |
| `@Assistant list groups` | `@Romi list groups` | Show registered groups |
| `@Assistant remember [fact]` | `@Romi remember I prefer dark mode` | Add to global memory |
...
```
User: @Romi remind me every Monday at 9am to review the weekly metrics
...
```
User: @Romi at 5pm today, send me a summary of today's emails
...
From any group:
- `@Romi list my scheduled tasks` - View tasks for this group
- `@Romi pause task [id]` - Pause a task
- `@Romi resume task [id]` - Resume a paused task
- `@Romi cancel task [id]` - Delete a task

From main channel:
- `@Romi list all tasks` - View tasks from all groups
- `@Romi schedule task for "Family Chat": [prompt]` - Schedule for another group
...
| "No groups registered" | Haven't added groups | Use `@Romi add group "Name"` in main |
