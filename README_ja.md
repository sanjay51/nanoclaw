...
トリガーワード（デフォルト：`@Romi`）でアシスタントに話しかけます：

```
@Romi 毎朝9時に営業パイプラインの概要を送って（Obsidian vaultフォルダにアクセス可能）
@Romi 毎週金曜に過去1週間のgit履歴をレビューして、差異があればREADMEを更新して
@Romi 毎週月曜の朝8時に、Hacker NewsとTechCrunchからAI関連のニュースをまとめてブリーフィングを送って
```

メインチャネル（セルフチャット）から、グループやタスクを管理できます：
```
@Romi 全グループのスケジュールタスクを一覧表示して
@Romi 月曜のブリーフィングタスクを一時停止して
@Romi Family Chatグループに参加して
```
