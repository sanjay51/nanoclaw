# Code Agent — Core Implementation Rules (Shared)

Product-agnostic implementation rules used by every code-assist skill. Product-specific `CODE-AGENT.md` files reference this and add product-specific notes (build commands, test URLs, repo path).
...
- If the build fails, fix the errors and re-run until it passes.
...
- If you make unrelated changes, revert them before the PR is raised.
- **If you discover bugs or improvements**, create a new backlog task with "AI Created" = Yes. Don't fix them in the current PR.

## UI Testing with Chrome (when the task touches UI)
